package com.example.shikiflow.presentation.viewmodel.manga.read.translations

import androidx.lifecycle.viewModelScope
import com.example.shikiflow.domain.usecase.GetChapterDataUseCase
import com.example.shikiflow.presentation.UiStateViewModel
import com.example.shikiflow.utils.DataResult
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.ExperimentalCoroutinesApi
import kotlinx.coroutines.flow.distinctUntilChanged
import kotlinx.coroutines.flow.filter
import kotlinx.coroutines.flow.flatMapLatest
import kotlinx.coroutines.flow.launchIn
import kotlinx.coroutines.flow.onEach
import kotlinx.coroutines.flow.update
import javax.inject.Inject

@OptIn(ExperimentalCoroutinesApi::class)
@HiltViewModel
class MangaChapterTranslationViewModel @Inject constructor(
    private val getChapterDataUseCase: GetChapterDataUseCase
): UiStateViewModel<ChapterTranslationsUiState>() {

    override val initialState: ChapterTranslationsUiState = ChapterTranslationsUiState()

    init {
        mutableUiState
            .filter { state ->
                state.chapterIds.isNotEmpty()
            }
            .distinctUntilChanged { old, new ->
                old.chapterIds == new.chapterIds && !new.isRefreshing
            }
            .flatMapLatest { state ->
                getChapterDataUseCase(state.chapterIds)
            }
            .onEach { result ->
                mutableUiState.update { state ->
                    when(result) {
                        DataResult.Loading -> {
                            state.copy(
                                isLoading = true,
                                isRefreshing = false,
                                errorMessage = null
                            )
                        }
                        is DataResult.Success -> {
                            state.copy(
                                chapterTranslations = result.data,
                                isLoading = false
                            )
                        }
                        is DataResult.Error -> {
                            state.copy(
                                errorMessage = result.message,
                                isLoading = false
                            )
                        }
                    }
                }
            }.launchIn(viewModelScope)
    }

    fun setChapterIds(chapterIds: List<String>) {
        mutableUiState.update { state ->
            state.copy(
                chapterIds = chapterIds
            )
        }
    }

    fun onRefresh() {
        mutableUiState.update { state ->
            state.copy(
                isRefreshing = true
            )
        }
    }
}